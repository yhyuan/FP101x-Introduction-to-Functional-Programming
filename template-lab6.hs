...
root :: Rose a -> a 
root (a :> _) = a --error "you have to implement root" 

children :: Rose a -> [Rose a]
children (_ :> xs) = xs --error "you have to implement children"
...
size :: Rose a -> Int
size (_ :> []) = 1
size (_ :> xs) = 1 + (foldl (\acc x -> acc + size(x)) 0 xs) -- error "you have to implement size"

leaves :: Rose a -> Int
leaves (_ :> []) = 1
leaves (_ :> xs) = (foldl (\acc x -> acc + leaves(x)) 0 xs) -- error "you have to implement leaves"
...
class Functor f => Foldable f where
  ...
  foldMap g a = fold $ fmap g a --(g x) `mappend`mempty  --  
  --foldMap  g (x :> xs) = (foldl (\acc y -> acc `mappend` (foldMap g y)) (g x) xs)
